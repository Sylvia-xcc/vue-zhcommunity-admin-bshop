    ...
    >
      ...
      <div v-if="type==3">
        ...
        <el-form-item style="margin-bottom: 40px;" label="拼团开始时间" prop="start_time" :required="true" :rules="{ required: true, message: '请选择拼团开始时间'}">
          <el-date-picker v-model="postForm.start_time" type="datetime" placeholder="选择拼团开始时间" />
        </el-form-item>
        <el-form-item style="margin-bottom: 40px;" label="拼团结束时间" prop="end_time" :required="true" :rules="{ required: true, message: '请选择拼团结束时间'}">
          <el-date-picker v-model="postForm.end_time" type="datetime" placeholder="选择拼团结束时间" />
        </el-form-item>
      </div>
      ...
      <el-form-item style="margin-bottom: 40px;" label="商品重量(kg)" prop="weight" :required="true">
        <el-input v-model="postForm.weight" placeholder="" clearable style="width:205px;" />
      </el-form-item>
      <el-form-item style="margin-bottom: 30px;" prop="thumb" label="商品图片：" :required="true">
        <Upload v-model="postForm.thumb" />
      </el-form-item>
      <el-form-item v-if="!roles" style="margin-bottom: 40px; text-align:left;" label="轮播图：">
        <UploadFile :file-list="postForm.fileList" @uploadfile="uploadFile" />
      </el-form-item>
      <el-form-item v-if="!roles" style="margin-bottom: 30px;" label="视频：">
      ...
      <el-form-item v-if="roles" label="使用规则：">
        <el-input v-model="postForm.instruc" type="textarea" rows="4" style="width:600px;" />
      </el-form-item>
      <el-form-item v-if="!roles" style="margin-bottom: 30px;" label="选择快递模板：" :required="true">
        <el-select v-model="postForm.fid" placeholder="请选择">
          <el-option :value="0" label="包邮" />
          <el-option v-for="item in freight" :key="item.value" :label="item.name" :value="item.id" />
        </el-select>
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        weight: [{ required: true, message: '请输入商品重量(kg)', trigger: 'blur' }],
        // store: [
    ...
    fetchData(id) {
      detailProduct(id)
        .then(res => {
          ...
          this.postForm.store = []
          if (res.merchant_class_id > 0) { this.postForm.store = [res.merchant_class_id] }
          console.log('---------', fileList)
          this.postForm.fileList = fileList
          this.postForm.weight = this.postForm.weigth==null?'1':this.postForm.weigth
